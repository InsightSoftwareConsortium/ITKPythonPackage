...
# aria2 is deliberately absent: conda-forge publishes no win-64 build, so
# `pixi global install aria2` always fails here with "No candidates were
# found" and prints a red "Error: couldn't install 1 environment" that looks
# like a real failure. The download below still prefers aria2c when one is
# already on PATH; otherwise it uses Invoke-WebRequest.
$globalPackages = @(
