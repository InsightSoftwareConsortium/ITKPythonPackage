...
def which_required(name: str) -> str:
    ...
    if not path:
        raise RuntimeError(f"MISSING: {name} not found in PATH; aborting until required executables can be found")
    return path
...
def get_git_id(repo_dir: Path) -> str:
    # 1. exact tag
    try:
        tag = run(["git", "describe", "--tags", "--exact-match"], cwd=repo_dir).stdout.strip()
        if tag:
            return tag
...
def compute_itk_package_version(itk_dir: Path, itk_git_tag: str, ipp_latest_tag: str) -> str:
    ...
    try:
        ...
        try:
            ...
            print(f"WARNING: Failed to checkout {itk_git_tag}, reverting to 'main': {e}")
            itk_git_tag = "main"
            run(["git", "checkout", itk_git_tag], cwd=itk_dir)
        desc = run(["git", "describe", "--tags", "--long", "--dirty", "--always"], cwd=itk_dir).stdout.strip()
...
def detect_platform() -> Tuple[str, str]:
    ...
    sysname = uname.sysname if uname else ("Windows" if os.name == "nt" else sys.platform)
    machine = uname.machine if uname else (os.environ.get("PROCESSOR_ARCHITECTURE", "").lower())
    os_name = "linux" if sysname.lower().startswith("linux") else (
        "darwin" if sysname.lower().startswith("darwin") or sys.platform == "darwin" else (
...
def default_manylinux(os_name: str, arch: str, env: Dict[str, str]) -> Tuple[str, str, str, str]:
    ...
    if os_name == "linux":
        if arch == "x64" and manylinux_version == "_2_34":
            ...
            raise RuntimeError(f"FAILURE: Unknown manylinux version {manylinux_version}")

        if arch == "x64":
            image_name = image_name or f"manylinux{manylinux_version}-{arch}:{image_tag}"
            container_source = container_source or f"docker.io/dockcross/{image_name}"
        elif arch == "aarch64":
            image_name = image_name or f"manylinux{manylinux_version}_{arch}:{image_tag}"
...
def main(argv: list[str]) -> int:
    ...
    ref_env_report_default = build_env_report if Path(build_env_report).exists() else None
    ...
    if reference_env_report and reference_env_report == build_env_report and Path(build_env_report).exists():
    ...
    # ITK repo handling
    itk_source_dir = Path(env.get("ITK_SOURCE_DIR", str(_ipp_dir / "ITK-source" / "ITK")))
    ipp_latest_tag = get_git_id(_ipp_dir)
    ipp_latest_pep440 = ipp_latest_tag[1:] if ipp_latest_tag.startswith("v") else ipp_latest_tag
    ...
    if not itk_source_dir.exists():
        ...
        run(["git", "clone", "https://github.com/InsightSoftwareConsortium/ITK.git", str(itk_source_dir)])
    ...
    itk_package_version = env.get("ITK_PACKAGE_VERSION") or compute_itk_package_version(itk_source_dir, itk_git_tag, ipp_latest_tag)
    ...
    if os_name == "linux":
        manylinux_version, image_tag, manylinux_image_name, container_source = default_manylinux(os_name, target_arch, env)
    ...
    if os_name == "darwin":
        ...
        build_vars = [
            "CC", "CXX", "FC", "CFLAGS", "CXXFLAGS", "FFLAGS", "CPPFLAGS",
            "LDFLAGS", "SDKROOT", "MACOSX_DEPLOYMENT_TARGET", "PKG_CONFIG_PATH",
            "PKG_CONFIG_LIBDIR", "LD_LIBRARY_PATH", "DYLD_LIBRARY_PATH"
        ]
        lines += [
            "",
